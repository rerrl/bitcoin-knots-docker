...
## LND + Lightning Terminal (Remote Access)

This stack adds an LND node and Lightning Terminal (LiT) web UI, both disabled by default. LND connects to bitcoind using cookie auth via the shared bitcoin data directory. LiT connects to LND in remote mode, exposing the web UI on port 8443.

### Enabling LND + LiT (disabled by default)

Both containers are gated behind a single Docker Compose profile — enable them together:

1. **Uncomment the profile line** in your `.env` file:

   ```
   COMPOSE_PROFILES=tor,lnd
   ```

   This tells Docker Compose to include the `lnd` profile (which covers both lnd and lit containers) on every `docker compose up`. If you don't want Tor, use just `COMPOSE_PROFILES=lnd`.
...
To disable them, comment out or remove `COMPOSE_PROFILES=lnd` from `.env`.

### Accessing Lightning Terminal

After starting the containers, LiT is available at `https://localhost:8443`. The TLS cert and admin macaroon are generated by LND on first start and persist in `./lnd-data/`.

### Important notes

- **LND uses cookie auth** — no rpcuser/rpcpass needed. The bitcoin data directory is mounted read-only into the LND container so it can read the `.cookie` file.
- **Back up `./lnd-data/`** — losing it means losing your channel state and generating new TLS certs and macaroons.
- **Both containers are disabled by default** — they only start if `COMPOSE_PROFILES=lnd` is set in your `.env` file.
